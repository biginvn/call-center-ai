...
if not all([AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY, AWS_REGION, S3_BUCKET]):
    ...
    raise ValueError("AWS environment variables (AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY, AWS_REGION, S3_BUCKET) must be set in .env")
...
async def dowloand_document(document_id: str, current_user: User = Depends(get_current_user)):
